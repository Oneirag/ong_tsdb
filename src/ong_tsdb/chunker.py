...
class Chunker(object):
    ...
    def __init__(self, freq, retention_chunks=None):
        ...
        if isinstance(freq, str):
            ...
            if period_type in ("S", "s"):
                ...
                multiplier = 60 * 60
            elif period_type.upper() in (
                "D",
                "C",
                "B",
            ):  # Days: calendar, custom and business
                multiplier = (
                    60 * 60 * 24
        ...
        self.retention_policy_chunks = retention_chunks or int(
            config("uncompressed_chunks", -1)
        )
    ...
    def compressed_by_policy(self, date_ts: float) -> bool:
        ...
        if self.retention_policy_chunks is None or self.retention_policy_chunks < 0:
            ...
            return (
                (time.time() - date_ts) / self.chunk_duration
            ) > self.retention_policy_chunks
    ...
    def chunk_name(self, timestamp, n_columns, compressed=None):
        ...
        return generate_filename_from_parts(
            path="",
    ...
    def getpos(self, timestamp):
        if isinstance(timestamp, np.ndarray):
            chunk_starts = (timestamp // self.chunk_duration) * self.chunk_duration
            return ((timestamp - chunk_starts) / self.tick_duration).astype(int)
        else:
            return int(
                (timestamp - self.chunk_timestamp(timestamp)) / self.tick_duration
            )
